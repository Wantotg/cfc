...
unrecognised arguments in both the chat and wiki-screen paths, before git is
consulted, through one shared acceptance decision. `TRACKER.md`,
`CHANGELOG.md`, `2c4df49`.
...
## ~~B-1.5.1-01a · A scheduled tick dies with `database is locked`~~ — CLOSED (v1.5.2, 2026-08-02)

**Closed 2026-08-02**, v1.5.2 — current-schema database opens no longer run
unneeded writes. A real two-process reproduction confirmed the pre-fix lock
failure and the shipped lock-free open. `TRACKER.md`, `CHANGELOG.md`,
`eb8db69`, `4e747bb`.
...
## ~~B-1.5.1-01b · A routine can do its work and leave no run-log line~~ — CLOSED (v1.5.2, 2026-08-02)

**Closed 2026-08-02**, v1.5.2 — every runner exit now appends the authoritative
run-log record before its best-effort transcript marker, and schedule ticks
contain each routine independently. `TRACKER.md`, `CHANGELOG.md`,
`076c806`, `4e747bb`.
...
## ~~B-06 · `ui.console` still rewrites `:word:` in chat content as an emoji~~ — CLOSED (v1.4.1, 2026-08-01)

**Closed 2026-08-01**, v1.4.1 — `ui.console` is now
`Console(markup=False, emoji=False)`, so chat content is printed literally.
`tests/test_ui.py` covers known shortcodes, including inside code-shaped text.
`CHANGELOG.md`, 9251de8.
...
## ~~B-1.3.1-02 · Typing while cfc is silently busy after a turn sends an unintended message~~ — CLOSED (v1.4.1, 2026-08-01)

**Closed 2026-08-01**, v1.4.1 — one shared finisher keeps the visible busy state
through titling and embedding, then offers the next prompt. `tests/test_turn_paths.py`.
`CHANGELOG.md`, 0cc952e.
...
## ~~B-05 · `run` from the routines screen uses a different model than `/routine`, silently~~ — CLOSED (v1.3, 2026-07-31)

**Closed 2026-07-31**, v1.3 — the routines screen now receives the chat model
and passes it to `do_routine`; a routine's own model pin still wins.
`tests/test_screens.py`, `CHANGELOG.md`.
