...
**Frozen at the v1.0 tag, 2026-07-29, and moved here 2026-07-30** — the split
`CHANGELOG.md` already made at the same tag, applied to this file once it too
got long enough that the earliest entries were costing more to scroll past
than they were worth reading in context of where cfc is now. Nothing here is
maintained; the live file is [`../ROADMAP.md`](../ROADMAP.md), which carries
v1.1 onward in the shape described at its own top.

This still uses the older, longer per-version shape — full prose, Cas's note,
the ASCII art — unrestyled, because restyling a record to a convention invented
afterwards destroys the one property it has: what was true when it was written.
