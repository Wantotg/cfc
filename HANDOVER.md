...
- **On-command `:routine` still uses the session model but nudges (y/n)** in `do_routine` when it isn't in `ROUTINE_MODELS`. It's a soft nudge, not a block — a thinking model is runnable by choice.
